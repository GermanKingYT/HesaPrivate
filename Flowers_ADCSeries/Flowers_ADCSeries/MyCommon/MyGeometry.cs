namespace Flowers_ADCSeries.MyCommon
{
    ...
    public static class MyGeometry
    {
        ...
        public static List<Vector3> GetCirclrPos(Vector3 startPos, float range)
        {
            ...
            for (var i = 1; i <= 360; i++)
            {
                ...
                var point = new Vector2(startPos.X + range * (float)Math.Cos(angle), startPos.Y + range * (float)Math.Sin(angle)).To3D();
        ...
        public class Polygon
        {
            ...
            public virtual void Draw(Color color, int width = 1)
            {
                for (var i = 0; i <= Points.Count - 1; i++)
                {
                    ...
                    Drawing.DrawLine(from.To3D(), to.To3D(), width, color.ToSharpDX());
                }
            }
