    ...
    internal class Vayne : MyLogic
    {
        ...
        private static void Combo()
        {
            if (ComboOption.UseR && R.IsReady() &&
                ObjectManager.Heroes.Enemies.Count(x => x.IsValidTarget(650)) >= ComboOption.GetSlider("ComboRCount") &&
                Me.HealthPercent <= ComboOption.GetSlider("ComboRHp"))
            {
                R.Cast();
            }
        ...
        {
            if (sender == null || !sender.IsEnemy || !sender.IsMelee() || sender.ObjectType != Me.ObjectType || Args.Target == null)
            {
                return;
            }
            ...
            {
                if (Args.Target.IsMe && sender.DistanceToPlayer() <= 300 && Me.HealthPercent <= 40)
                {
        ...
        {
            foreach (var target in ObjectManager.Heroes.Enemies.Where(x => !x.IsDead && x.IsValidTarget(E.Range) && !x.HasBuffOfType(BuffType.SpellShield)))
            {
                if (CondemnCheck(Me.ServerPosition, target))
                {
                    E.CastOnUnit(target, true);
